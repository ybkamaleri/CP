...
t1 <- kable(tab3, align = "c", format = 'latex')
t2 <- kable(tab4, align = "c", format = 'latex')

cat(c("\\begin{table}[h] \\centering ",
      t1,
      "\\hspace{1cm} \\centering",
      t2,
      "\\caption{Fødselsår} \\end{table}"))
## beholder graphic device åpne
opts_knit$set(global.device = TRUE)

library(gridExtra)
tb1 <- tableGrob(tab3)
tb2 <- tableGrob(tab4)
grid.draw(gtable_combine(tb1, tb2, along = 1))
